...
public class OAuth2Controller {
    ...
    // Una implementación real usaría almacenamiento persistente para los códigos de autorización
    ...
    /**
     * Endpoint para obtener tokens (Authorization Code, Refresh Token, Client Credentials)
     */
    ...
    public Mono<ResponseEntity<TokenResponse>> token(
        ...
        // Validación básica de cliente (en una implementación real se haría más completa)
        if (clientId == null) {
        ...
        switch (grantType) {
            ...
            default:
                return Mono.just(ResponseEntity.badRequest().body(
                        new TokenResponse("unsupported_grant_type", null, null, 0, null)));
        }
    }
    
    private Mono<ResponseEntity<TokenResponse>> handleAuthorizationCodeGrant(String code, String redirectUri, String clientId) {
        // Verificar código de autorización
        if (!authorizationCodes.containsKey(code)) {
            return Mono.just(ResponseEntity.badRequest().body(
                    new TokenResponse("invalid_grant", null, null, 0, null)));
        }
        ...
        if (!authRequest.clientId.equals(clientId) || !authRequest.redirectUri.equals(redirectUri)) {
            return Mono.just(ResponseEntity.badRequest().body(
                    new TokenResponse("invalid_grant", null, null, 0, null)));
        }
        ...
        return Mono.just(ResponseEntity.ok(
                new TokenResponse("bearer", accessToken, refreshToken, expirationTime, authRequest.scope)));
    }
    ...
    private Mono<ResponseEntity<TokenResponse>> handleRefreshTokenGrant(String refreshToken, String clientId) {
        ...
        return Mono.just(ResponseEntity.ok(
                new TokenResponse("bearer", accessToken, refreshToken, expirationTime, null)));
    }
        ...
        return authUseCase.authenticateUser(username, password)
                .map(user -> {
                    ...
                    return ResponseEntity.ok(
                            new TokenResponse("bearer", accessToken, refreshToken, expirationTime, "read write"));
                })
    ...
    private Mono<ResponseEntity<TokenResponse>> handleClientCredentialsGrant(String clientId, String clientSecret) {
        // En una implementación real, validaríamos el clientId y clientSecret contra la base de datos
        ...
        if ("gateway-client".equals(clientId) && "gateway-client-secret".equals(clientSecret)) {
            ...
            return Mono.just(ResponseEntity.ok(
                    new TokenResponse("bearer", accessToken, null, expirationTime, "read write")));
        }
        
        return Mono.just(ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(
                new TokenResponse("invalid_client", null, null, 0, null)));
    }
    ...
    private static class AuthorizationRequest {
        ...
        public AuthorizationRequest(String clientId, String redirectUri, String scope, String state) {
...
// Clase para la respuesta de token
class TokenResponse {
